...
public:
    ...
    const std::unordered_map< std::string,std::string >& get_buffer(){return _bfbs_buffer;};
    const char *get_buffer( std::string name ){return _bfbs_buffer[name].c_str();};
    bool load_bfbs_file( const char *file );
    bool load_bfbs( const char *name,  const char *content, size_t size );
