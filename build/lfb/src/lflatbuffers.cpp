...
/* load a binary flatbuffers schema file */
bool lflatbuffers::load_bfbs( const char *name_, const char *content, size_t length )
{
    /* Both dirname() and basename() may modify the contents of path, so it
     may be desirable to pass a copy when calling one of these functions.
     */
    char file_name[PATH_MAX];
    snprintf( file_name,PATH_MAX,"%s",name_ );

    const char *name = basename( file_name );
    /* if the file already loaded,consider it need to update,not dumplicate */

    char contents[length];
    memcpy((void*)contents, (const void*)content, length);
    std::string &bfbs = _bfbs_buffer[name] = std::string(contents, length);

    flatbuffers::Verifier encode_verifier(reinterpret_cast<const uint8_t *>(bfbs.c_str()), length);
    if ( !reflection::VerifySchemaBuffer(encode_verifier) )
    {
        _bfbs_buffer.erase( name_ );

        ERROR_WHAT( "invalid flatbuffers binary schema name:" );
        ERROR_APPEND( name_ );
...
static int load_bfbs_file( lua_State *L )
{
    ...
    char file_name[PATH_MAX];
    snprintf( file_name,PATH_MAX,"%s",path );
    const char *name = ::basename( file_name );
    const char* bfbs = (*lfb)->get_buffer(name);
    lua_pushstring( L, bfbs);
    return 2;
}


static int load_bfbs( lua_State *L )
{
    class lflatbuffers** lfb =
    (class lflatbuffers**)luaL_checkudata( L, 1, LIB_NAME );
    if ( lfb == NULL || *lfb == NULL )
    ...
    size_t length =  luaL_checknumber( L,3 );
    const char* content = luaL_checklstring(L, 4, &length);
    if ( !(*lfb)->load_bfbs(name, (const char*)content, length ) )
    {
        return luaL_error( L,(*lfb)->last_error());
    }
...
static int loaded_bfbs( lua_State *L )
{
    class lflatbuffers** lfb = (class lflatbuffers**)luaL_checkudata( L, 1, LIB_NAME );
    if ( lfb == NULL || *lfb == NULL )
    {
        return luaL_error( L, "encode argument #1 expect %s", LIB_NAME );
    }
    auto buffers = (*lfb)->get_buffer();
    ...
    for (auto i = buffers.begin(); i != buffers.end(); ++i) {
        ...
        lua_pushlstring(L, i->first.c_str(), i->first.length());
        lua_rawset( L,-3 );
        lua_pushstring(L, "contents");
        lua_pushlstring(L, i->second.c_str(), i->second.length());
        lua_rawset( L,-3 );
...
int luaopen_lua_flatbuffers(lua_State *L)
{
    ...
    lua_pushcfunction(L, load_bfbs);
    lua_setfield(L, -2, "load_bfbs");

    lua_pushcfunction(L, loaded_bfbs);
...
{
    return luaopen_lua_flatbuffers(L);
}
